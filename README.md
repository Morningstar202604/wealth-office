...
| 入口 | 内容 |
|---|---|
| 仪表盘 | 总市值 / 累计盈亏 / 本月结余 / 储蓄率、资产分布与支出结构图表、近 6 个月收支趋势、应急金进度、负债与订阅、持仓明细（红涨绿跌、可排序）、风险提示横幅；支持定时自动刷新与大金额缩写 |
| 问答 | 多会话管理（新建 / 重命名 / 删除）、真流式 SSE 回答、语音提问、回答导出（复制 / 下载 Markdown）、按当前数据自动生成追问建议、尾部关键数字摘要与风险提示、可展开"分析过程" |
| 记账 | 录入 / 删除持仓、流水（带完整日期）、负债；红涨绿跌 |
| 设置 | 外观（浅色 / 深色 / 跟随系统、四套品牌色、界面密度、界面动效）、功能开关（语音提问 / 回答导出 / 分析过程展开 / 建议入口 / 仪表盘自动刷新 / 大金额缩写）、账本假设（月收入 / 应急金目标 / 必要支出类别 / 储蓄率目标）、行情源、每日晨报、数据管理（恢复示例 / 导出全部数据 JSON 备份）、访问口令 |
