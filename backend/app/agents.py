...
DISCLAIMER = (
    "以上为基于组合与账本数据的洞察，不构成投资建议；涉及决策的建议已标记待人审。"
)
...
def _usage(src: str) -> dict:
    ...
    HITL 扣留/批准步骤没有调模型，记 0/0，不虚增"模板兜底"。
    """
    if src in ("llm",):
        return {"llm_calls": 1, "llm_fallbacks": 0}
    if src == "hitl":
        return {"llm_calls": 0, "llm_fallbacks": 0}
    return {"llm_calls": 0, "llm_fallbacks": 1}
...
def _rule_route(question: str) -> tuple[str, str]:
    ...
    if hit_m and hit_l:
        return (
            "both",
            f"同时命中投资类({','.join(hit_m[:2])})与收支类({','.join(hit_l[:2])})关键词",
        )
    if hit_m:
...
async def market_agent(state: dict, config: RunnableConfig) -> dict:
    ...
    pnl_word = "浮盈" if pf["total_pnl"] >= 0 else "浮亏"
    if top:
        breach = (
            f"，超过 {conc['threshold_pct']}% 阈值"
            if top["pct"] > conc["threshold_pct"]
            else f"（阈值 {conc['threshold_pct']}%）"
        )
        fallback = (
            f"组合总市值 {pf['total_market_value']:,.0f} 元，{pnl_word} "
            f"{abs(pf['total_pnl']):,.0f} 元（{pf['total_pnl_pct']}%）。"
            f"最大单一持仓 {top['name']} 占 {top['pct']}%{breach}。"
        )
    else:
        fallback = "组合为空。"
    llm_sys = (
    ...
    text, src = await narrate(
        llm_sys,
        json.dumps({"portfolio": pf, "concentration": conc}, ensure_ascii=False),
        fallback,
    )
...
async def ledger_agent(state: dict, config: RunnableConfig) -> dict:
    ...
    view = {
        ...
        "debt_monthly": debts["monthly_total"],
        "dti_pct": debts["dti_pct"],
...
async def risk_agent(state: dict, config: RunnableConfig) -> dict:
    ...
    text, src = await narrate(
        ...
        json.dumps(
            {
                "flags": flags,
                "market": state.get("market_view"),
                "ledger": state.get("ledger_view"),
            },
            ensure_ascii=False,
        ),
        fallback,
...
async def finalize(state: dict, config: RunnableConfig) -> dict:
    await _emit(
        config, agent="finalize", label="文书", phase="start", detail="汇总成文并分级"
    )
    ...
    if level.startswith("L2") and cfg.get("hitl", True):
        ...
        await _emit(
            config, agent="finalize", phase="start", detail="人审通过，开始成文"
        )
